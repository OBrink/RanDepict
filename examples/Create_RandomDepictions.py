...
def main():
	...
	with random_depictor() as depictor:
	    depictor.batch_depict_save(smiles, 3, file_in+'_not_augmented', depiction_img_shape, chembl,10)
	    
	with random_depictor() as depictor:
		depictor.batch_depict_augment_save(smiles, 3, file_in+'_augmented', depiction_img_shape, chembl,10)

	with open(file_in+"_paths","w") as fp:
		for i in range(len(chembl)):
			for j in range(3):
				fp.write(file_in+'_not_augmented/'+chembl[i]+"_"+str(j)+","+smiles[i]+"\n")
			for k in range(3):
				fp.write(file_in+'_augmented/'+chembl[i]+"_"+str(k)+","+smiles[i]+"\n")		
