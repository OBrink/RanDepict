...
class random_depictor:
    ...
    def get_nonexisting_image_name(
        self, path: str = "./tmp/", format: str = "PNG"
    ) -> str:
    ...
    def get_random_indigo_rendering_settings(
        self, shape: Tuple[int, int, int] = (299, 299, 3)
    ) -> Indigo:
    ...
    def depict_and_resize_indigo(
        self, smiles: str, shape: Tuple[int, int, int] = (299, 299, 3)
    ) -> np.array:
    ...
    def depict_and_resize_rdkit(
        self, smiles: str, shape: Tuple[int, int] = (299, 299, 3)
    ) -> np.array:
        """This function takes a smiles str and an image shape. It renders the chemical structures
    ...
    def get_random_label_position(self, width: int, height: int) -> Tuple:
        ...
        if random.choice([True, False]):
            ...
            x_range = list(range(0, int(0.1 * width))) + list(
                range(int(0.9 * width), width)
            )
        else:
            y_range = list(range(0, int(0.1 * height))) + list(
                range(int(0.9 * height), height)
            )
    ...
    def add_straight_arrows_to_structure(self, image: np.array) -> np.array:
        ...
        for _ in range(random.choice(range(1, 3))):
            ...
            new_arrow_image_shape = (
                int(width / 2 * random.choice(np.arange(1.0, 1.5, 0.1))),
                int(height / 10 * random.choice(np.arange(1.0, 1.5, 0.1))),
            )
